...
int main(void) {
    ...
    Func HSerializer("hSerializer", Place::Host), HLoader("hLoader", PLACE);
    Func RSerializer("rSerializer", Place::Host), RLoader("rLoader", PLACE);
    Func Unloader("unloader", PLACE), Deserializer("deserializer", Place::Host);

    Hap.isolate_producer_chain(H, HSerializer, HLoader)
       .isolate_producer_chain({R, delta, zeta, eta, alpha_match, alpha_gap, beta_match, beta_gap}, RSerializer, RLoader);
    Result.isolate_consumer_chain(Unloader, Deserializer).min_depth(16);

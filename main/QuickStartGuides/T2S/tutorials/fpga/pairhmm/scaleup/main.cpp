...
int main(void) {
    ...
    RLoader.remove(jj, hhh).min_depth(16); 
    RFeeder.buffer(RLoader, oi, BufferStrategy::Double);
    RFeeder.scatter(RLoader, ii).min_depth(16);
    HSerializer.remove(ii, rrr, oi, rr);
    HLoader.remove(ii, rrr).min_depth(128);
    HFeeder.buffer(HLoader, oi, BufferStrategy::Double);
    HFeeder.scatter(HLoader, jj).min_depth(128);

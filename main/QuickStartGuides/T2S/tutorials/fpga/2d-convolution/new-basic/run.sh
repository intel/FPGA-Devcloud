...
if [ $1 = "emulator" ]; then
  ...
  env  PRAGMAUNROLL=1 INTEL_FPGA_OCL_PLATFORM_NAME="$EMULATOR_PLATFORM_NAME"  CL_CONTEXT_EMULATOR_DEVICE_INTELFPGA=1 CL_CONFIG_CHANNEL_DEPTH_EMULATION_MODE=strict BITSTREAM=$CODE_STORE_DIR/a.aocx  AOC_OPTION="-march=emulator  -board=$FPGA_BOARD" $CODE_STORE_DIR/a.out
else

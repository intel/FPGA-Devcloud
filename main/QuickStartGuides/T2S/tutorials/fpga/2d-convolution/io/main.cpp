...
#define FUNC_DECL HALIDE_TYPE, {A}, Place::Device
    ...
    Func xSerializer(Place::Host), xLoader(Place::Device), 

         wSerializer(Place::Host), wLoader(Place::Device), 

         zUnloader(Place::Device), zDeserializer(Place::Host);
    ...
    Buffer<TYPE> results = zDeserializer.realize({RR, CC, OO, OUTR, OUTC, OUTO}, target);

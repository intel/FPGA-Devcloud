...
Func matrix_multiply() {
    ...
    // (Optional): Check the layout of the input data to be safe
    // -----------------------------------------------------------------------------------------------
    ...
    Func kk_k_EQ_0  ("kk_k_EQ_0",   Bool(1), {P}, Place::Device),
         kk_k_EQ_max("kk_k_EQ_max", Bool(1), {P}, Place::Device),
         kk_EQ_0    ("kk_EQ_0",     Bool(1), {P}, Place::Device),
         A("A", TYPE, {P}, Place::Device), // Name (optional), return type, arguments and Place.
    ...
    kk_k_EQ_0(P)   = select(jjj == 0, kk_k_eq_0,     kk_k_EQ_0(P_jjj_minus_1));
    kk_k_EQ_max(P) = select(jjj == 0, kk_k_eq_max, kk_k_EQ_max(P_jjj_minus_1));
    kk_EQ_0(P)     = select(jjj == 0, kk_eq_0,         kk_EQ_0(P_jjj_minus_1));

    // Compute UREs.
    A(P)   = select(jjj == 0, a(total_k, total_i), A(P_jjj_minus_1));
    B(P)   = select(iii == 0, b(total_j, total_k), B(P_iii_minus_1));
    C(P)   = select((kkk == 0) && kk_k_EQ_0(P), 0,
                    select(kkk == 0, select(kk_EQ_0(P), C(P_k_minus_1), C(P_kk_minus_1)), C(P_kkk_minus_1))
                   ) + A(P) * B(P);
    c(P_c) = select((kkk == KKK - 1) && kk_k_EQ_max(P), C(P));

    // Put all the UREs inside the same loop nest. Now the first URE (kk_k_EQ_0) represents all the UREs.
    kk_k_EQ_0.merge_ures(kk_k_EQ_max, kk_EQ_0, A, B, C, c);

    // Explicitly set the loop bounds
    kk_k_EQ_0.set_bounds(kkk, 0, KKK, jjj, 0, JJJ, iii, 0, III)
             .set_bounds(kk,  0, KK,  jj,  0, JJ,  ii,  0, II)
             .set_bounds(k,   0, K,   j,   0, J,   i,   0, I);

    kk_k_EQ_0.space_time_transform(kkk, jjj, iii)
             .vectorize(kkk);
    ...
    kk_k_EQ_0.isolate_producer_chain({a, kk_k_eq_0, kk_k_eq_max, kk_eq_0}, aLoader, aFeeder);
    ...
    kk_k_EQ_0.isolate_producer_chain(b, bSerializer, bLoader, bFeeder);

...
Func matrix_multiply() {
    ...
    #define P_kk_minus_1  kkk + KKK - 1, jjj,     iii,     kk - 1,      jj, ii, k,     j, i // To be used only when kkk == 0 and kk != 0
    #define P_k_minus_1   kkk + KKK - 1, jjj,     iii,     kk + KK - 1, jj, ii, k - 1, j, i // To be used only when kkk == 0, kk == 0 and k != 0
...
int main() {
    ...
    target.set_features({Target::IntelFPGA, Target::Debug}); // To execute on an Intel FPGA device attached to the host. Show debug info

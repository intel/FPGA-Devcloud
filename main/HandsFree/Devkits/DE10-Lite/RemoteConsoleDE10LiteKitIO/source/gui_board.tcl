...
##### Make GUI objects
	...
	lassign [add_board_canv_obj 	$sw_img_path $sw_base_x $sw_base_y $sw_num_obj "normal" \
	...
	grid [text .msg -background white -height 4 -width 51 -foreground blue -relief ridge \
		...
		-padx 1 -pady 10 -column 3 -row 2 -columnspan 3 -rowspan 2

	grid [scrollbar .scroll -command {.msg yview}] -padx 1 -pady 10 -column 6 -row 2\
		-columnspan 1 -rowspan 2

	grid [button .resetTimeout  -text "Reset Timer"  -height 1 -width 12 -command "set session_started 0"] \
		 -pady 10 -column 3 -row 4 -columnspan 1

	grid [checkbutton .gui_in_toggle_box -text "Input from GUI" -variable GUI_input  -background white -font {Courier -17} \
	-command {if { $GUI_input } {
				set PBs $PBs_start
				set SWs $SWs_start
   				update_input_objects $PBs $pb_num_obj $SWs $sw_num_obj
			 }}] -pady 10 -column 4 -row 4 -columnspan 1
	 

	.gui_in_toggle_box select

	grid [checkbutton .board_out_toggle_box -text "Output to Board" -variable Board_output -background white -font {Courier -17} ] -pady 10 -column 5 -row 4 -columnspan 1

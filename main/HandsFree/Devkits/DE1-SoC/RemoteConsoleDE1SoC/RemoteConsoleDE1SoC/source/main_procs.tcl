	...
	while { $linked == 0 } {
		...
		if { [pipe_wait $gui_connect_pipe "connect"] == 0 } {
			...
			if { $server_credentials != 1 } {
				...
				set cdf_path 	        [lindex $server_credentials 3]
				if {$cdf_path == "Optional - Use for SoC Kits"} {
					set cdf_path ""
				}
				puts $sof_path
		...
		if { [attempt_device_pgm $device $sof_path $cdf_path] == "success" } {
	...
	foreach line $data {
		if { ( "[string index $line 1]"==")" ) || ( "[string index $line 2]"==")" ) || ( "[string index $line 3]"==")" ) } {
			...
			if { [string match *Unable* "$line_cut"] == 1 } {
				continue;
			} elseif { ([string match *USB-Blaster* "$line_cut"] == 1) || ([string match *DE-SoC* "$line_cut"] == 1) } {
...
proc attempt_device_pgm {device_choice sof_file cdf_path} {
	...
	if { $cdf_path == 0 } {
		...
		##### Command to program the device in Windows
		set sys_cmd_pgm_windows "$env(QUARTUS_ROOTDIR)bin64/quartus_pgm.exe $cable_command \"$cdf_path\""
		##### Command to program the device in Linux
		set sys_cmd_pgm_linux "$env(QUARTUS_ROOTDIR)/linux64/quartus_pgm $cable_command \"$cdf_path\""
